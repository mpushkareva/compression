...
def main(args):
    ...
    # train model
    SCHEDULE_EPOCHS = [20, 5, 5] #[50, 5, 5] # divide lr by 10 after each number of epochs
#     SCHEDULE_EPOCHS = [100, 50, 50] # divide lr by 10 after each number of epochs
    ...
    for num_epochs in SCHEDULE_EPOCHS:
        ...
        for epoch in range(num_epochs):
            check_accuracy_train(model, loader_val)
    ...
    print('Saving model')
    torch.save(model.state_dict(), 'ResNet-34_20_epoch_weights')
...
def check_accuracy_train(model, loader):
    num_correct = 0
    num_samples = 0
    ...
    for X, y in loader:
        ...
        num_samples_train += preds.size(0)
    ...
    print('Eval mode %d / %d correct (%.2f)' % (num_correct, num_samples, 100 * acc))
    print('Train mode %d / %d correct (%.2f)' % (num_correct_train, num_samples_train, 100 * acc_train))
